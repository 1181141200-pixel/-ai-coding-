...
Page({
  ...
  async loadPage(targetPage, done) {
    ...
    try {
      const res = await linesService.list({ page: targetPage, pageSize: this.data.pageSize }, {
  ...
  // 统一跳转至独立登录页
  goLogin() {
    const redirect = '/pages/lines/index';
  ...
  goDetail(e) {
    ...
    if (!id) return;
    wx.navigateTo({ url: `/pages/lines/detail?id=${id}` });
  },
});