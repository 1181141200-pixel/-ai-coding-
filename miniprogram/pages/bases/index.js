...
Page({
  ...
  async loadPage(targetPage, done) {
    ...
    try {
      const res = await basesService.list({ page: targetPage, pageSize: this.data.pageSize }, {
  ...
  // 统一跳转至独立登录页
  goLogin() {
    const redirect = '/pages/bases/index';
  ...
  goDetail(e) {
    ...
    if (!id) return;
    wx.navigateTo({ url: `/pages/bases/detail?id=${id}` });
  },
});